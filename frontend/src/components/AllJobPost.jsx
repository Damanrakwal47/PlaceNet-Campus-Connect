...
// function AllJobPost() {
//   document.title = 'PCC | Job Listings';
...
const isEligible = (student, eligibility) => {
  if (!student || !student.studentProfile) return false;

  const profile = student.studentProfile;

  // Ensure SGPA values are numbers
  const sgpas = Object.values(profile.SGPA || {}).map(Number).filter(val => !isNaN(val));
  const cgpa = sgpas.length > 0 ? (sgpas.reduce((a, b) => a + b, 0) / sgpas.length) : 0;

  const meetsCGPA = cgpa >= Number(eligibility.cgpa || 0);
  const meetsYear = Number(profile.year) === Number(eligibility.year);
const meetsGap = eligibility.gap === 0 ? !JSON.parse(profile.gap) : true;
  const meetsBacklogs = Number(profile.liveKT || 0) <= Number(eligibility.backlogs || 0);
  const meetsTenth = Number(profile.pastQualification?.ssc?.percentage || 0) >= Number(eligibility.tenthPercentage || 0);
  const meetsTwelfth = Number(profile.pastQualification?.hsc?.percentage || 0) >= Number(eligibility.twelfthPercentage || 0);

  console.log(`Checking Job: ${eligibility?.jobTitle || 'Unknown Title'}`, {
    cgpa, meetsCGPA,
    year: profile.year, meetsYear,
    gap: profile.gap, meetsGap,
    liveKT: profile.liveKT, meetsBacklogs,
    tenth: profile.pastQualification?.ssc?.percentage, meetsTenth,
    twelfth: profile.pastQualification?.hsc?.percentage, meetsTwelfth
  });
  ...
  useEffect(() => {
    ...
    .then(res => {
      setCurrentUser({
        ...res.data,
        studentProfile: res.data.studentProfile
      });
  ...
  const fetchJobs = async () => {
    try {
      ...
      console.log("All Job Data:", response.data.data);
                  ...
                  <tbody>
                    {jobs?.length > 0 ? (
                      jobs
                        ?.filter((job) => {
                          // Log job eligibility
                          console.log(`Checking Job: ${job.jobTitle}`, job.eligibility);

                          if (currentUser.role !== 'student') return true;
                          return isEligible(currentUser, job.eligibility);
                        })
                        .map((job, index) => {
                          const isMatched = job?.applicants?.find(student => student.studentId == currentUser.id);
                          ...
                          return (
                            ...
                            >
                              ...
                              <td>
                                <b>
                                  {companies[job?.company] || <Placeholder as="p" animation="glow"><Placeholder xs={12} /></Placeholder>}
                                </b>
                              </td>
                              <td>{job?.jobTitle}</td>
                              <td>{job?.salary}</td>
