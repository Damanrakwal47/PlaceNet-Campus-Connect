...
function UserDetails() {
  document.title = 'PCC | Complete Profile';
  ...
  if (file) {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/user/upload-photo`, formData);
      setUserData({ ...userData, profile: response.data.url }); // <- use response.data.url here
      if (response.data?.msg) {
        setToastMessage(response.data.msg);
        setShowToast(true);
      }
    } catch (error) {
      const msg = error.response?.data?.msg || error.message || "Error uploading photo";
      setToastMessage(msg);
      setShowToast(true);
      console.error('Error uploading photo:', error);
    }
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
                    ...
                    <div className="px-2 py-3 flex flex-col items-center gap-4 my-1">
                      <Col className="w-32 md:w-44 lg:w-52">
                        ...
                        <Image src={userData?.profile} roundedCircle alt="helloo"/>
                      </Col>
